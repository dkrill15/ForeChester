...
training_data = [['name', 'round', 'segment', 'revision date', 'stock out', 'performance', 'size',
...
for i, row in preds.iterrows():
    ...
    if key in previous_round_info:
        ...
        send = [
            float(row['list price']), row['segment'], float(row['revision date']),
            ...
            float(row['sales budget']), float(o['accessibility']), 45, 1, int(row['round']) - 1, update_age
        ]
    elif row['round'] > 0:
        #new product
        send = [
            float(row['list price']), row['segment'], float(
                row['revision date']),
            0, float(row['mtbf']), 0,
            0, float(row['size']), 0,
            float(row['performance']), float(
                row['promo budget']), .25,
            float(row['sales budget']), .25, 45, 1, int(row['round']) -1, update_age
            ]

    else:
        continue

    bonk = send + [row['dec cust survey']]
    training_data.append(bonk)
    wscores, averages, test_val = forechester.score(*send)
...
tooltip_a = plugins.PointLabelTooltip(actuals, labels=[str(i) for i in preds.index])
tooltip_p = plugins.PointLabelTooltip(predictions, labels=[str(i) for i in preds.index])
